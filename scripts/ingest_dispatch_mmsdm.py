...
def run(start: date, end: date, output: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "completed_days": requested_days if not incomplete else requested_days - len({item["day"] for item in incomplete}),
        "complete_1440_row_days": requested_days - len({item["day"] for item in incomplete}),
        "failed_days": 0,
        "rows": len(combined),
