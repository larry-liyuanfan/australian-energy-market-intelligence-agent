...
`/metrics` exports request status, latency buckets, typed-tool status/recovery, citation totals, provider/backend identity and loaded row/chunk gauges. A post-deployment gate over **100 real-data loopback requests** (five regions × coverage/event/forecast/BESS, concurrency 4) achieved 100/100 HTTP, task, citation and raw-tool success; P50/P95/max latency was **395/2,380/3,593 ms**. This is a bounded single-host loopback service check, not a public-network SLA or an independent answer-quality benchmark.
