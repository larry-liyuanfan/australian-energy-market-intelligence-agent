...
The pinned **Qwen3-VL-Embedding-2B** adapter was then run over all **87 pages** of the official AEMO Q4 2024 report on an A100 MIG slice. Encoding produced 2,048-D vectors in **75.39 s**, with **4.66 GB peak CUDA allocation** (job `29504579`, exit `0:0`). On 14 same-author caption-derived queries, the visual route reached **MRR 0.9286 / Recall@5 1.00**; text was already **1.00 / 1.00**. The first fusion result exposed a real ranking bug: metadata bonuses overrode RRF rank evidence. Restricting numeric/modality metadata to exact-score tie-breaking raised fusion from MRR **0.5702 to 0.9643**, but still did not beat text (fusion-minus-text bootstrap interval **-0.1071 to 0.0000**). This is full-corpus systems evidence and a documented negative quality result—not a claim that a 2B model is large, state of the art, or economically beneficial. See the [compact run artifact](artifacts/public/multimodal_qwen_q4_2024_20260822.json).
